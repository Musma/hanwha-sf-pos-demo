...
- 사이드바 대메뉴: 실행계획 관리·소조·중조·주판론지·대조·의장·도장·PE·워크프론트·시스템 관리 아코디언으로 구성한다. 워크프론트 하위 메뉴는 워크프론트 점검과 에코텍 자체 품질지표로 구성한다. 초기 상태는 전부 접힘이며, 라벨 우측 ▸/▾로 상태를 표시하고 클릭 또는 Enter/Space로 토글한다. 화면 진입 시 해당 화면이 속한 대메뉴는 자동으로 펼친다.
